...
from kerosene.training.trainers import SimpleTrainer, Model
...
class ModelCheckpointIfBetterTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self._module_mock = mockito.mock(nn.Module)
        ...
        self._model = Model(self.MODEL_NAME, self._module_mock, self._criterion_mock,
    ...
    @mock.patch("kerosene.training.trainers.Model.optimizer_state", new_callable=PropertyMock)
    @mock.patch("kerosene.training.trainers.Model.model_state", new_callable=PropertyMock)
    @mock.patch("kerosene.training.trainers.Model.valid_loss", new_callable=PropertyMock)
    def test_should_not_save_model_with_higher_valid_loss(self, valid_loss_mock, model_state_mock,
        ...
        self._handler_mock = mockito.spy(
            Checkpoint(self.SAVE_PATH, lambda model: model.valid_loss, 0.01, MonitorMode.MIN))
        ...
        self._trainer_mock.models = [self._model]
    ...
    @mock.patch("kerosene.training.trainers.Model.optimizer_state", new_callable=PropertyMock)
    @mock.patch("kerosene.training.trainers.Model.model_state", new_callable=PropertyMock)
    @mock.patch("kerosene.training.trainers.Model.valid_metrics", new_callable=PropertyMock)
    def test_should_save_optimizer_with_higher_valid_metric(self, valid_metrics_mock, model_state_mock,
                                                            optimizer_state_mock):
        self._handler_mock = mockito.spy(
            Checkpoint(self.SAVE_PATH, lambda model: model.valid_metrics, 0.01, MonitorMode.MAX))
