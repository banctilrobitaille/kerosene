...
class ModelConfiguration(Configuration, HtmlConfiguration):
    def __init__(self, model_name, model_type, model_params, optimizer_config: Configuration,
                 scheduler_config: Configuration, criterions_configs: List[Configuration],
                 metrics_configs: List[Configuration], gradient_clipping_config: Configuration):
    ...
    def from_dict(cls, model_name, config_dict):
        try:
            ...
            criterion_configs = [Configuration.from_dict(criterion, config_dict["criterion"][criterion]) for criterion
                                 in config_dict["criterion"].keys()]

            if "metrics" in config_dict.keys():
                metric_configs = [Configuration.from_dict(metric, config_dict["metrics"][metric]) for metric in
